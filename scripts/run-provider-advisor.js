...
const ASK_PROVIDER_ENV = "OMX_ASK_PROVIDER";

function usage() {
	console.error('Usage: omx ask <claude|gemini> "<prompt>"');
	console.error(
		"Legacy direct usage: node scripts/run-provider-advisor.js <claude|gemini> <prompt...>",
	);
	console.error(
		'                 or: node scripts/run-provider-advisor.js claude --print "<prompt>"',
	);
	console.error(
		'                 or: node scripts/run-provider-advisor.js gemini --prompt "<prompt>"',
	);
}
...
function parseArgs(argv) {
	let rest = [...argv];
	let provider = (process.env[ASK_PROVIDER_ENV] || "").toLowerCase();
	const providerRaw = (rest[0] || "").toLowerCase();

	if (providerRaw in PROVIDER_BINARIES) {
...
function ensureBinary(binary) {
	...
	if (probe.error && probe.error.code === "ENOENT") {
		...
		console.error(
			`[ask-${binary}] Missing required local CLI binary: ${binary}`,
		);
		console.error(
			`[ask-${binary}] Install/configure ${binary} CLI, then verify with: ${verify}`,
		);
		process.exit(1);
	}
}
...
main().catch((error) => {
	console.error(
		`[run-provider-advisor] ${error instanceof Error ? error.message : String(error)}`,
	);
	process.exit(1);
});
